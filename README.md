# Hi, I’m Abubaker Hassan 👋

📊 **Data Analyst | Power BI Developer | SQL | DAX | Excel | CDMP®**

I’m a Data Analyst & Power BI Developer | CDMP® Certified, with a strong foundation in data governance, data quality, and KPI-driven reporting. Specializing in the end-to-end development of scalable business intelligence solutions and governed data architectures. I bridge the gap between complex raw data and executive-level decision-making by combining advanced Power BI development with rigorous Data Management principles.

---

## 👨‍💻 About Me
- Strong focus on **data accuracy, performance, and usability**
- Experience turning business requirements into **actionable dashboards**
- Continuous learner in **data analytics, BI, and data management best practices**
- Interested in **data governance, security, and scalable reporting**

---

## 🛠️ Skills & Tools

**Data Sources**
- SQL Databases, Excel, CSV, APIs  

**Data Preparation & Transformation**
- Power Query (M Language)
- Advanced Excel
- SQL  

**Data Modeling & Analytics**
- Star & Snowflake Schema  
- DAX (Measures, Calculated Columns)
- Row-Level Security (RLS)
- Time Intelligence Functions  

**Data Visualization & BI**
- Power BI (Reports, Dashboards, Power BI Service)

---

## 📂 Featured Projects
📊 [**Finance Analysis Dashboard**]([https://app.powerbi.com/view?r=eyJrIjoiZWZhNTYyZDktODM4MC00MjkzLThlY2ItZjYyZGJiYTljYTczIiwidCI6IjJiYjZlNWJjLWMxMDktNDdmYi05NDMzLWMxYzZmNGZhMzNmZiIsImMiOjl9](https://app.powerbi.com/view?r=eyJrIjoiNmNkY2ZiNGYtOTI4Yi00MjU3LTk0NWYtMmIzYzMzM2Q5N2Q5IiwidCI6IjJiYjZlNWJjLWMxMDktNDdmYi05NDMzLWMxYzZmNGZhMzNmZiIsImMiOjl9))
Interactive Power BI Finance Analysis Dashboard for analyzing financial transactions, customer segments, transaction trends, fees, taxes, and regional performance using DAX, Power Query, and Star Schema modeling.

---

## 🎓 Education
**B.Sc. (Honors) in Management Information Systems**  
University of Science & Applied Studies

---

## 📜 Certifications
- **CDMP® Associate – Certified Data Management Professional** (DAMA International®)
- [**Explore CDMP® Certificate**](https://certificates.cdmp.info/e1bce9e4-f811-4a6d-9bc4-c24637655136#acc.D6NSyUvQ)

---

## 🚀 What I’m Working On
- Building **interactive and executive-level dashboards**
- Enhancing **data security** using Row-Level Security (RLS)
- Improving **performance and optimization** of Power BI reports
- Collaboration and knowledge sharing in analytics projects

---

## 📫 Connect with Me
- 💼 LinkedIn: [Abubaker Hassan](https://www.linkedin.com/in/abubaker-hassan-cdmp)
- 🌐 GitHub: [Abubaker Hassan](https://www.github.com/abubakerhassan)

---

⭐ *Feel free to explore my repositories and dashboards*
